...
fn main()
{
    let mut client = TcpClient::new("192.168.6.15:502");
...
fn reading_coils(client: &mut TcpClient) {
    ...
    let response = client.read_coils(3, 3);
...
fn reading_input_registers(client: &mut TcpClient) {
    ...
    let response = client.read_input_registers(0, 100);
    println!("Response IR: {:?}", response);
}
...
fn write_registers(client: &mut TcpClient) {
    ...
    let response = client.write_single_register(10, 10);
    println!("Response WSRE: {:?}", response);   

    let response = client.write_multiple_registers(0, vec!(23456, 77, 2356, 252,2020));
